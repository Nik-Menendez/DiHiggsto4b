...
def draw_comparison_plots(original, target, variables, original_weights, tag, norm=True):
    ...
    for idx, column in enumerate(variables, 1):
        ...
        # ksresults.append(ks)
        # i += 1
    plt.savefig("plots/distr_%s.png" % tag)
    # return ksresults 
...
# parser.add_argument('--input',   dest='input',   help='Input ROOT file',                        required = True)
# parser.add_argument('--output',  dest='output',  help='Output file for the reweighting model',  required = True)
...
# dataset   = arrs.pandas.df(entrystop = 100000) # not all entries - only for debug!
...
# ## here are the variables that the reweighting must see
...
reweighter_base = reweight.GBReweighter      (n_estimators=100, learning_rate=0.1, max_depth=4, min_samples_leaf=400, gb_args={'subsample': 0.5} )
...
print "... saving 3 b tag events with the background modelling to the output file: ", outputfile_3b
...
# dataset_3b = dataset_3b.reset_index(drop=True)
